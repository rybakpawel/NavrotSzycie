...
const Quantity = ({ id, quantity, overallQuantity }) => {
    const dispatch = useDispatch();

    const handleChangeQuantity = (calculation) => {
        dispatch(changeQuantity(id, overallQuantity, calculation));
    };
    ...
    return (
        <div className='quantity'>
            ...
            <img className={`quantity__change-quantity ${quantity >= overallQuantity ? 'quantity__change-quantity--grey' : ''}`} onClick={() => handleChangeQuantity('+')} src={plus} alt='dodaj' />
        </div>
    )
};
